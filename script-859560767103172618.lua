-- ======================
-- PIXEL PAINTER PRO (V6.1 - FPS CUSTOM & GIF SYNC)
-- ======================
...
local function parseTextToArt(text)
    ...
    for idx, hex in text:gmatch("Pixel%s+(%d+)[^=]*=%s*([A-Fa-f0-9]+)") do
        ...
        if n and n >= 1 and n <= TOTAL then art[n] = hex:upper() end
    end
    return art
end

-- ======================
-- LÓGICA DE PINTURA (ESTÁTICA E GIF)
-- ======================

local function runPaint(updateBar, setProgress)
    ...
    if selectedArt.isGif then
        ...
        setProgress("🎬 Rodando GIF (".. (selectedArt.fps or 10) .." FPS)")
        ...
        while animationActive and selectedArt and selectedArt.isGif do
            for _, frameText in ipairs(selectedArt.frames) do
                ...
                for pixId, hex in pairs(frameData) do
                    packet[tostring(logicalToBoardIndex(pixId))] = hex
                end
                ...
                -- Espera dinâmica baseada no FPS que você escolheu no bot
                task.wait(1 / (selectedArt.fps or 10))
            end
            task.wait(0.01)
        end
    else
        -- Pintura Estática Inteligente
        local artData = parseTextToArt(selectedArt.map)
        local needUpdate = {}
        
        setProgress("🔍 Escaneando...")
        for i, desiredHex in pairs(artData) do
            local bIdx = logicalToBoardIndex(i)
            local p = board:FindFirstChild(tostring(bIdx))
            if p and p:IsA("BasePart") then
                local target = Color3.fromHex(desiredHex)
                if (p.Color.R - target.R)^2 + (p.Color.G - target.G)^2 + (p.Color.B - target.B)^2 > 0.001 then
                    table.insert(needUpdate, {id = tostring(bIdx), color = desiredHex})
                end
            end
        end
        ...
        if #needUpdate == 0 then
            ...
            for i, data in ipairs(needUpdate) do
                ...
                if #packet >= 128 or i == #needUpdate then
                    pcall(function() remote:InvokeServer(packet) end)
                    packet = {}
                    updateBar(i/#needUpdate)
                    setProgress("Pintando... " .. math.floor((i/#needUpdate)*100) .. "%")
                    task.wait(0.02)
                end
            end
            setProgress("Arte Feita ✅")
...
local function createGUI()
    ...
    main.Size = UDim2.new(0, 280, 0, 325); main.Position = UDim2.new(1, -10, 0.5, 0); main.AnchorPoint = Vector2.new(1, 0.5); main.BackgroundColor3 = Color3.fromRGB(20, 20, 25); main.Visible = false; main.Active = true; main.Draggable = true; Instance.new("UICorner", main)
    ...
    local progTxt = Instance.new("TextLabel", progBg); progTxt.Size = UDim2.new(1, 0, 1, 0); progTxt.BackgroundTransparency = 1; progTxt.Text = "Aguardando..."; progTxt.TextColor3 = Color3.new(1,1,1); progTxt.TextSize = 10; progTxt.Font = Enum.Font.GothamBold
    ...
    -- Loop do Preview Animado
    task.spawn(function()
        while true do
            if selectedArt and selectedArt.isGif and selectedArt.frames then
                for _, frameText in ipairs(selectedArt.frames) do
                    if not selectedArt or not selectedArt.isGif then break end
                    local data = parseTextToArt(frameText)
                    for pixId, hex in pairs(data) do
            ...
            if currentPlaylist == "Todas" or art.playlist == currentPlaylist then
                ...
                btn.MouseButton1Click:Connect(function()
                    ...
                    if not art.isGif then
                        ...
                        for i = 1, TOTAL do pixels[i].BackgroundColor3 = Color3.fromHex(data[i] or "282828") end
                    end
                end)
            end
        end
    ...
    for _, name in ipairs(PLAYLISTS) do
        ...
        b.MouseButton1Click:Connect(function() currentPlaylist = name; loadArts() end)
    end

    startBtn.MouseButton1Click:Connect(function()
        runPaint(function(p) progBar.Size = UDim2.new(p,0,1,0) end, function(t) progTxt.Text = t end)
    end)
